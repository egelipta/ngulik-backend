...
async def check_permissions(req: Request, security_scopes: SecurityScopes, token=Depends(OAuth2)):
    ...
    # ---------------------------------------验证权限-------------------------------------------------------------------
    # Check whether the user is true and effective, or has been disabled
    check_user = await User().get_or_none(id=user_id)
    if not check_user or check_user.user_status != 1:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Users do not exist or have been banned by administrators!",
            headers={"WWW-Authenticate": f"Bearer {token}"},
        )
    # 判断是否设置了权限域
    if security_scopes.scopes:
        ...
        if not user_type and security_scopes.scopes:
            ...
            print(f"Checking permission for :{security_scopes.scopes} hasilnya {is_pass}" )
            if not is_pass:
                raise HTTPException(
